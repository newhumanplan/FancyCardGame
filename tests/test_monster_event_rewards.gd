...
func _run_tests() -> void:
	...
	test_pve_win_applies_monster_item_and_skill_rewards()
...
func test_pve_win_applies_monster_item_and_skill_rewards() -> void:
	...
	_assert_eq(int(result.get("gold_reward", 0)), 3, "Viper win grants gold")
	_assert_eq(int(result.get("xp_reward", 0)), 2, "Viper win grants XP")
	_assert_true((summary.get("items", []) as Array).size() >= 1, "Viper reward grants an item from its pool")
	_assert_true((summary.get("skills", []) as Array).size() >= 1, "Viper reward grants a skill from its pool")
	_assert_true(GameManager.selected_hero.skills.size() >= 1, "monster skill reward is stored on the hero")
