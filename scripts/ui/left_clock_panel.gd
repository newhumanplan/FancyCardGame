...
const INACTIVE_HOUR_COLOR: Color = Color(0.20, 0.17, 0.12, 0.92)
...
func refresh(day: int, hour: int, prestige: int, pvp_wins: int) -> void:
	...
	phase_label.text = "Hour %d/%d  %s" % [hour_index + 1, HOUR_COUNT, _get_phase_name(hour_index)]
...
func _create_hour_dots() -> void:
	...
	for index in range(HOUR_COUNT):
		...
		dot.add_theme_stylebox_override("panel", _create_dot_style(_get_hour_color(index, -1), false))
		...
		label.add_theme_color_override("font_color", Color(0.90, 0.82, 0.62, 0.88))
		dot.add_child(label)
...
func _create_dot_style(color: Color, highlighted: bool) -> StyleBoxFlat:
	...
	style.border_color = Color(0.95, 0.82, 0.38, 1.0) if highlighted else Color(0.46, 0.36, 0.18, 0.82)
